    ...
    try {
      ...
      if (interaction.options.getSubcommand() === "analyze") {
        ...
        try {
          ...
          const confidenceField = formatConfidenceBlock(
            analysis.confidence,
            analysis.notes,
          );
          ...
          if (sheetDetails.error) {
            console.error(
              "[snail] Auto-save disabled due to sheet error:",
              sheetDetails.error.message,
            );
          }
          ...
          if (sheetIdForSave) {
            try {
              ...
              if (isNotFound) {
                console.warn(
                  "[snail] Stored sheet missing, creating a new one...",
                );
                try {
                  ...
                  if (newSheetId && newSheetId !== sheetIdForSave) {
                    ...
                    await database.setSheetsConsent(
                      userId,
                      guildId,
                      true,
                      newSheetId,
                ...
                console.error(
                  "[snail] Failed to auto-save stats:",
                  err.message,
                );
              }
            }
          ...
          if (savedToSheet) {
            ...
            if (sheetDetails.sheetUrl) {
              sheetMessages.push(
                `📊 Saved to Google Sheet: ${sheetDetails.sheetUrl}`,
              );
            }
            if (sheetDetails.created) {
              sheetMessages.push(
                "🆕 A new sheet was provisioned for you automatically.",
              );
            }
            ...
            embed.setFooter({
              text: "✅ Auto-saved to your Google Sheet • Analyzed with gpt-4o",
            });
          ...
          const collector = reply.createMessageComponentCollector({
            filter: (i) =>
              i.customId === `enable-sheets-${statId}` && i.user.id === userId,
            time: 60_000,
            max: 1,
          });
          ...
          collector.on("collect", async (i) => {
            ...
            try {
              ...
              const provisionNote = sheet.existed
                ? "📊 **Google Sheets re-enabled!**"

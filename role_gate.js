...
const REVIEW_CHANNEL_NAME  = process.env.REVIEW_CHANNEL_NAME  || 'builders_review_inbox';

const ROLES = [
  { name: process.env.SNAIL_ROLE_NAME || 'snail_team',  label: 'Snail Team', color: 0xF3B13D, customId: 'gate_toggle:snail_team',  desc: 'Access **snail_lab/** (math + analysis)' },
  { name: process.env.TESTERS_ROLE_NAME || 'testers',   label: 'Testers',    color: 0x8BE37F, customId: 'gate_toggle:testers',     desc: 'File bugs & run checks in **testing_grounds/**' },
  { name: process.env.OBSERVERS_ROLE_NAME || 'observers',label: 'Observers', color: 0x9CA3AF, customId: 'gate_toggle:observers',   desc: 'Read-only visibility across most categories' },
];
...
const commands = [
  new SlashCommandBuilder()
    ...
    .addStringOption(o => o.setName('channel').setDescription(`Channel (default: ${WELCOME_CHANNEL_NAME})`)),
].map(c => c.toJSON());
...
async function ensureRole(guild, name, color) {
  ...
  return guild.roles.cache.find(r => r.name === name) || guild.roles.create({ name, color, reason: 'Role gate setup' });
}

async function ensureReviewChannel(guild) {
  await guild.channels.fetch();
  let ch = guild.channels.cache.find(c => c.type === ChannelType.GuildText && c.name === REVIEW_CHANNEL_NAME);
  if (ch) return ch;

  // Try to place under dev_ops if it exists
  const devOpsCat = guild.channels.cache.find(c => c.type === ChannelType.GuildCategory && c.name === 'dev_ops');
...
function panelRows() { return [toggleRow(), requestRow()]; }
function panelBody() {
  const lines = [PANEL_MESSAGE, ''];
  for (const r of ROLES) lines.push(`• **${r.label}** — ${r.desc}`);
  lines.push('• **Builders** — elevated permissions to structure and ship features (request via button).');
  return lines.join('\n');
}

async function ensurePanelMessage(guild, channelName) {
  await guild.channels.fetch();
  const ch = guild.channels.cache.find(c => c.type === ChannelType.GuildText && c.name === channelName);
  if (!ch) throw new Error(`Channel not found: ${channelName}`);

  const msgs = await ch.messages.fetch({ limit: 25 }).catch(()=>null);
  const existing = msgs?.find(m => m.author?.id === guild.client.user.id && m.components?.length && m.components[0].components?.some?.(b => b.customId?.startsWith('gate_toggle:')));
...
client.on('interactionCreate', async (ix) => {
  try {
    ...
    if (ix.isChatInputCommand() && ix.commandName === 'setup_gate') {
      ...
      const created = [];
      for (const r of ROLES) created.push(await ensureRole(guild, r.name, r.color));
      const buildersRole = await ensureRole(guild, BUILDERS_ROLE_NAME, 0x00C2FF);
      const me = await guild.members.fetchMe();
      const tooLow = [buildersRole, ...created].filter(role => me.roles.highest.comparePositionTo(role) <= 0);
      if (tooLow.length) {
        await ix.editReply(`⚠️ Move my highest role above: ${tooLow.map(r => `\`${r.name}\``).join(', ')}`);
        return;
      }
    ...
    if (ix.isButton() && ix.customId === 'gate_request:builders') {
      ...
      const reason = new TextInputBuilder()
        .setCustomId('reason').setLabel('Why do you need builders?')
        .setStyle(TextInputStyle.Paragraph).setRequired(true).setMaxLength(500);
      const links = new TextInputBuilder()
        .setCustomId('links').setLabel('Links (PRs, examples) — optional')
        .setStyle(TextInputStyle.Short).setRequired(false).setMaxLength(200);
      modal.addComponents(new ActionRowBuilder().addComponents(reason), new ActionRowBuilder().addComponents(links));
    ...
    if (ix.isModalSubmit() && ix.customId === 'builders_request_modal') {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          { name: 'User', value: `<@${ix.user.id}> (${ix.user.id})`, inline: false },
      ...
      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(approveId).setStyle(ButtonStyle.Success).setLabel('Approve'),
        new ButtonBuilder().setCustomId(denyId).setStyle(ButtonStyle.Danger).setLabel('Deny')
      );
    ...
    // Approve / Deny
    if (ix.isButton() && (ix.customId.startsWith('builders_approve:') || ix.customId.startsWith('builders_deny:'))) {
      ...
      try {
        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId('disabled_approve').setStyle(ButtonStyle.Success).setLabel('Approve').setDisabled(true),
          new ButtonBuilder().setCustomId('disabled_deny').setStyle(ButtonStyle.Danger).setLabel('Deny').setDisabled(true),
        );

...
function getConfiguredFolderId(explicitFolderId) {
  const envValue =
    process.env.SHEETS_PARENT_FOLDER_ID || process.env.GOOGLE_SHEETS_FOLDER_ID;
...
async function ensureDriveMetadata({
  drive,
  fileId,
  folderId,
  name,
...
async function createSnailStatsSheet(options) {
  const { title, userId, username, guildId, guildName, parentFolderId } =
    options;
  ...
  try {
    ...
    const appProperties = buildAppProperties({
      userId,
      guildId,
      username,
      guildName,
    ...
    await sheets.spreadsheets.values.update({
      ...
      requestBody: {
        values: [
          ...
          [
            "Instructions",
            "Stats are automatically saved from /snail analyze command",
          ],
        ],
      },
    });
...
async function ensureSnailStatsSheet(options) {
  ...
  try {
    ...
    if (existing) {
      ...
      return {
        ...
        url:
          ...
          `https://docs.google.com/spreadsheets/d/${existing.id}`,
    ...
    console.error(
      "[sheets-creator] Error locating existing spreadsheet:",
      err.message,
    );
  }
...
async function saveStats(options) {
  ...
  try {
    ...
    console.log(
      `[sheets-creator] Saved stats for ${username} to ${spreadsheetId}`,
    );
    return true;
  } catch (err) {
...
async function logAnalysis(options) {
  ...
  try {
    ...
    console.log(
      `[sheets-creator] Logged analysis for ${username} to ${spreadsheetId}`,
    );
    return true;
  } catch (err) {
...
module.exports.createSnailStatsSheet =
  require("./sheets-drive-create").createSnailStatsSheet;

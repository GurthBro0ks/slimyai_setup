...
function hasApiKey() { return !!process.env.GOOGLE_SHEETS_API_KEY; }
...
function _readSAFromEnv() {
  ...
  try { return JSON.parse(raw); } catch {
    ...
    } catch { throw new Error('GOOGLE_SERVICE_ACCOUNT_JSON is neither JSON nor base64-JSON.'); }
  }
}
...
function _saAuth() {
  ...
  if (!credsInline && !keyFile) throw new Error('No SA creds. Set GOOGLE_APPLICATION_CREDENTIALS or GOOGLE_SERVICE_ACCOUNT_JSON.');
  if (keyFile && !fs.existsSync(keyFile)) throw new Error(`Service account file not found at ${keyFile}`);
...
async function _fetchMeta_apiKey(spreadsheetId) {
  ...
  if (!res.ok) throw new Error(`Meta fetch failed (API key): ${res.status} ${await res.text()}`);
  return res.json();
}
...
async function _fetchMeta(spreadsheetId) {
  ...
  const meta = hasServiceAccount() ? await _fetchMeta_sa(spreadsheetId) : await _fetchMeta_apiKey(spreadsheetId);
...
async function fetchRange({ spreadsheetId, tabName, gid, rangeA1 = 'A:K' }) {
  ...
  if (hasServiceAccount()) {
    const sheets = _saAuth();
    const { data } = await sheets.spreadsheets.values.get({ spreadsheetId, range: a1 });
    const values = (data.values || []).map(row => row.map(_maybeNum));
    return { range: data.range || a1, values };
  }

  if (!hasApiKey()) throw new Error('No auth set. Provide GOOGLE_APPLICATION_CREDENTIALS (SA) or GOOGLE_SHEETS_API_KEY.');
  ...
  const res = await fetch(url);
  if (!res.ok) throw new Error(`values.get failed (API key): ${res.status} ${await res.text()}`);
  const json = await res.json();
  const values = (json.values || []).map(row => row.map(_maybeNum));
  return { range: json.range || a1, values };
}

async function fetchKeyedByFirstCol({ spreadsheetId, tabName, gid, rangeA1 = 'A:K' }) {
...
function resolveGidAlias(input) {
  ...
  const aliases = (() => {
    try { return JSON.parse(process.env.SHEETS_GID_ALIASES || '{}'); } catch { return {}; }
  })();

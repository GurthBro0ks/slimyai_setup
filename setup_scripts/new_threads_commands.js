...
function toSnakeShort(s, maxWords = 6, maxLen = 36) {
  const cleaned = (s || '')
    .toLowerCase().replace(/[^a-z0-9\s_-]/g, ' ').replace(/\s+/g, ' ').trim();
...
async function nextId(forum, prefix) {
  const active = await forum.threads.fetchActive().catch(()=>({threads:[]}));
  const archived = await forum.threads.fetchArchived().catch(()=>({threads:[]}));
...
async function findForumByName(guild, name) {
  ...
  return guild.channels.cache.find(c => c.type === ChannelType.GuildForum && c.name === name);
}
...
const commands = [
  new SlashCommandBuilder()
    ...
    .addStringOption(o => o.setName('title').setDescription('short description').setRequired(true))
    .addStringOption(o => o.setName('severity').setDescription('impact').addChoices(
      { name:'low', value:'low' }, { name:'med', value:'med' }, { name:'high', value:'high' }
    ).setRequired(true))
    .addStringOption(o => o.setName('tag').setDescription('forum tag').addChoices(
      { name:'open', value:'open' }, { name:'needs_test', value:'needs_test' }, { name:'duplicate', value:'duplicate' }
    ))
    .addStringOption(o => o.setName('env').setDescription('OS/app/channel'))
    .addStringOption(o => o.setName('expected').setDescription('what should happen'))
    .addStringOption(o => o.setName('actual').setDescription('what actually happened')),
  new SlashCommandBuilder()
    .setName('new_calc')
    .setDescription('Create a calc thread in snail_math_den_forum_exe')
    .addStringOption(o => o.setName('title').setDescription('short math topic').setRequired(true))
    .addStringOption(o => o.setName('confidence').setDescription('confidence').addChoices(
      { name:'low', value:'low' }, { name:'med', value:'med' }, { name:'high', value:'high' }
    ).setRequired(true))
    .addStringOption(o => o.setName('tag').setDescription('forum tag').addChoices(
      { name:'needs_data', value:'needs_data' }, { name:'approx', value:'approx' },
      { name:'verified', value:'verified' }, { name:'model_update', value:'model_update' }
    ))
    .addStringOption(o => o.setName('inputs').setDescription('inputs (units!)'))
    .addStringOption(o => o.setName('approach').setDescription('formula/approach')),
  new SlashCommandBuilder()
    .setName('new_feature')
    .setDescription('Create a feature request in feature_requests_forum_exe (pings testers)')
    .addStringOption(o => o.setName('title').setDescription('short feature name').setRequired(true))
    .addStringOption(o => o.setName('status').setDescription('initial status tag').addChoices(
      { name:'idea', value:'idea' }, { name:'approved', value:'approved' },
      { name:'in_progress', value:'in_progress' }, { name:'deferred', value:'deferred' }
    ))
    .addStringOption(o => o.setName('problem').setDescription('what hurts today?'))
    .addStringOption(o => o.setName('proposal').setDescription('what we want built'))
    .addStringOption(o => o.setName('impact').setDescription('who benefits / measurable outcome'))
    .addStringOption(o => o.setName('alternatives').setDescription('other approaches considered')),
  new SlashCommandBuilder()
    .setName('new_test')
    .setDescription('Create a test thread in a phase forum (test_v#_desc)')
    .addIntegerOption(o => o.setName('phase').setDescription('phase 1–4').addChoices(
      { name:'1', value:1 }, { name:'2', value:2 }, { name:'3', value:3 }, { name:'4', value:4 }
    ).setRequired(true))
    .addStringOption(o => o.setName('title').setDescription('short test focus').setRequired(true))
    .addIntegerOption(o => o.setName('version').setDescription('override version number (optional)'))
    .addStringOption(o => o.setName('result').setDescription('initial tag').addChoices(
      { name:'needs_review', value:'needs_review' }, { name:'pass', value:'pass' }, { name:'fail', value:'fail' }
    ))
...
client.on('interactionCreate', async (ix) => {
  try {
    ...
    const tagIdByName = (forum, n) => forum?.availableTags?.find(t => t.name.toLowerCase() === n.toLowerCase())?.id;
      ...
      const thread = await forum.threads.create({
        ...
        message: {
          content: [
            ...
            approach ? `**formula/approach**: ${approach}` : '**formula/approach**:',
            '**sample calc**:',
            '**result**:',
    ...
    if (ix.commandName === 'new_feature') {
      ...
      const thread = await forum.threads.create({
        ...
        message: {
          content,
          allowedMentions: testersRole ? { parse: [], roles: [testersRole.id] } : undefined,
        },
        appliedTags: tagId ? [tagId] : undefined,
      });
    ...
    if (ix.commandName === 'new_test') {
      ...
      const version = givenVersion ? String(givenVersion) : await nextVersion(forum);
      const short = toSnakeShort(title);
      ...
      const body = [
        ...
        '',
        '**steps**:',
        steps ? steps.split(';').map(s => s.trim()).filter(Boolean).map((s,i)=>`${i+1}. ${s}`).join('\n') : '1.\n2.\n3.',
        '',
        '**expected**:',
        '**actual**:',

...
const TEXT_TOPICS = {
  // planning
  announcements_log:
    "release notes • goals • locked decisions • minimal chatter",
  // testing_grounds
  playground_tmp:
    "freefire @mention tests • ephemeral chatter • rate-limit yourselves pls",
  ...
  try {
    ...
    const findText = (n) =>
      guild.channels.cache.find(
        (c) => c.type === ChannelType.GuildText && c.name === n,
      );
    const findForum = (n) =>
      guild.channels.cache.find(
        (c) => c.type === ChannelType.GuildForum && c.name === n,
      );
    ...
    for (const [name, topic] of Object.entries(TEXT_TOPICS)) {
      ...
      if (!ch) {
        console.warn(`⚠️ missing text channel: ${name}`);
        continue;
      }
    ...
    for (const [forumName, topic] of Object.entries(FORUM_TOPICS)) {
      ...
      if (!forum) {
        console.warn(`⚠️ missing forum: ${forumName}`);
        continue;
      }
      try {
    ...
    for (const t of FORUM_TEMPLATES) {
      ...
      if (!forum) {
        console.warn(`⚠️ missing forum: ${t.forum}`);
        continue;
      }
      ...
      const tagId = (forum.availableTags || []).find(
        (x) => x.name.toLowerCase() === (t.tag || "").toLowerCase(),
      )?.id;
      const options = {
        name: t.name,

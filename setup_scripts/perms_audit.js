...
const WANT = {
  team_corner: {
    ...
    denySend: ["observers.zip"],
  },
  bot_lab: {
    view: ["verified_exe"],
    create: ["testers.txt", "coder.js"],
    denySend: ["observers.zip"],
  },
  testing_grounds: {
    view: ["verified_exe"],
  ...
  snail_lab: {
    view: ["math_expert.7z", "snail_team.iso", "coder.js"],
    denyView: ["verified_exe"],
  },
  chat: {
    view: ["verified_exe"],
    send: [
...
client.once("ready", async () => {
  ...
  for (const [catName, want] of Object.entries(WANT)) {
    ...
    if (!cat) {
      console.log(`⚠️ missing category: ${catName}`);
      continue;
    }
    ...
    const check = (roleName, perms) => {
      ...
      if (!r) return ok(false, `role missing: ${roleName}`);
      const p = cat.permissionsFor(r);
      return ok(
        perms.every((f) => p?.has(f)),
        `${roleName} has ${perms.map((x) => Object.keys(PermissionsBitField.Flags).find((k) => PermissionsBitField.Flags[k] === x)).join("+")}`,
      );
    };
  ...
  for (const f of forums) {
    const ch = guild.channels.cache.find(
      (c) => c.type === ChannelType.GuildForum && c.name === f,
    );
    ok(!!ch, f);
  }

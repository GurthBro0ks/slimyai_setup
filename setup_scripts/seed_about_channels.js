...
const CONTENT = {
  ...
  modes_and_commands_md: [
    ...
    "- `/ask <question>` – ephemeral answer (clean for testing)",
...
client.once("ready", async () => {
  try {
    ...
    for (const [chanName, topic] of Object.entries(TOPICS)) {
      ...
      if (!ch) {
        console.warn(`⚠️ missing channel: ${chanName}`);
        continue;
      }
      ...
      try {
        ...
        console.warn(`topic set failed for ${chanName}:`, e.message);
      }

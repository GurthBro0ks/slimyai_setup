        ...
        get(target, prop) {
          ...
          if (prop === 'renameSync') {
            return function(oldPath, newPath) {
              if (oldPath.includes('data_store.json')) oldPath = oldPath.replace('data_store.json', 'data_store_test.json');
...
async function unitTests() {
  ...
  await test('addMemo() creates memo with unique ID', async () => {
    const memo = await mem.addMemo({ userId: 'user1', guildId: 'guild1', content: 'Test note' });
    assert.ok(memo._id);
  ...
  await test('addMemo() persists to database', async () => {
    await mem.addMemo({ userId: 'user1', guildId: 'guild1', content: 'Test note' });
    const db = readTestDb();
  ...
  await test('listMemos() returns user memos', async () => {
    ...
    await mem.addMemo({ userId: 'user2', guildId: 'guild1', content: 'Note 3' });
  ...
  await test('listMemos() respects guild isolation', async () => {
    await mem.addMemo({ userId: 'user1', guildId: 'guild1', content: 'Guild note' });
  ...
  await test('deleteMemo() returns false for non-existent ID', async () => {
    const deleted = await mem.deleteMemo({ id: 'nonexistent', userId: 'user1' });
...
async function integrationTests() {
  ...
  await test('Full remember → export → forget flow', async () => {
    // Step 1: Set consent
    await mem.setConsent({ userId: 'user1', guildId: 'guild1', allowed: true });
    const consent = await mem.getConsent({ userId: 'user1', guildId: 'guild1' });
    assert.strictEqual(consent, true);

    // Step 2: Add memo
    const memo = await mem.addMemo({ userId: 'user1', guildId: 'guild1', content: 'Integration test' });
    assert.ok(memo._id);
    ...
    const memosAfter = await mem.listMemos({ userId: 'user1', guildId: 'guild1' });
...
async function edgeCaseTests() {
  ...
  await test('Very long content (10000 chars)', async () => {
    ...
    const memo = await mem.addMemo({ userId: 'user1', guildId: 'guild1', content: longText });
  ...
  await test('Unicode and multilingual content', async () => {
    ...
    const memo = await mem.addMemo({ userId: 'user1', guildId: 'guild1', content: unicode });
    assert.strictEqual(memo.content, unicode);
  });

  await test('Null guildId (DM context)', async () => {
    const memo = await mem.addMemo({ userId: 'user1', guildId: null, content: 'DM note' });
  ...
  await test('Rapid sequential operations', async () => {
    const promises = [];
    for (let i = 0; i < 10; i++) {
      promises.push(mem.addMemo({ userId: 'user1', guildId: 'guild1', content: `Note ${i}` }));
    }
...
async function raceConditionTests() {
  ...
  await test('Concurrent addMemo operations', async () => {
    const operations = [];
    for (let i = 0; i < 20; i++) {
      operations.push(mem.addMemo({ userId: `user${i % 5}`, guildId: 'guild1', content: `Concurrent note ${i}` }));
    }
    ...
    if (db.memos.length < 20) {
      throw new Error(`Race condition detected: Expected 20 memos, got ${db.memos.length}`);
    }
  ...
  await test('Concurrent addMemo + deleteMemo', async () => {
    // Add one memo first
    const memo = await mem.addMemo({ userId: 'user1', guildId: 'guild1', content: 'To delete' });

    // Now run concurrent add + delete
    const operations = [
      mem.addMemo({ userId: 'user1', guildId: 'guild1', content: 'New note 1' }),
      mem.addMemo({ userId: 'user1', guildId: 'guild1', content: 'New note 2' }),
      mem.deleteMemo({ id: memo._id, userId: 'user1' }),
      mem.addMemo({ userId: 'user1', guildId: 'guild1', content: 'New note 3' }),
    ];
...
async function errorHandlingTests() {
  ...
  await test('Corrupted database file recovery', async () => {
    ...
    const mem = loadMemoryModule();
    const memo = await mem.addMemo({ userId: 'user1', guildId: 'guild1', content: 'After corruption' });
  ...
  await test('Missing database file auto-creation', async () => {
    ...
    const mem = loadMemoryModule();
    const memo = await mem.addMemo({ userId: 'user1', guildId: 'guild1', content: 'New DB' });
...
async function dataIntegrityTests() {
  ...
  await test('Memo IDs are unique across rapid creation', async () => {
    const ids = new Set();
    for (let i = 0; i < 50; i++) {
      const memo = await mem.addMemo({ userId: 'user1', guildId: 'guild1', content: `Test ${i}` });
      if (ids.has(memo._id)) {
  ...
  await test('Deleted memos are actually removed from file', async () => {
    ...
    assert.strictEqual(found, undefined, 'Deleted memo should not exist in file');
  });

  await test('User isolation: Users cannot delete other users memos', async () => {
    const user1Memo = await mem.addMemo({ userId: 'user1', guildId: 'guild1', content: 'User 1' });
    const user2Memo = await mem.addMemo({ userId: 'user2', guildId: 'guild1', content: 'User 2' });

    // User1 tries to delete User2's memo
    const deleted = await mem.deleteMemo({ id: user2Memo._id, userId: 'user1' });
    assert.strictEqual(deleted, false);

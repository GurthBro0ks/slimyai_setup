// lib/thresholds.js
/**
 * Threshold resolver for club analytics verification.
 * Priority: DB overrides → per-guild env → default env → fallback
 */
  ...
  if (database.isConfigured()) {
    try {
      const rows = await database.query(
        `SELECT key_name, value FROM guild_settings
         WHERE guild_id = ? AND key_name IN ('warn_total_low', 'warn_total_high')`,
        [guildId],
      );

      for (const row of rows) {
        if (row.key_name === "warn_total_low" && row.value) {
          low = Number(row.value);
        }
        if (row.key_name === "warn_total_high" && row.value) {
          high = Number(row.value);
        }
      }
    } catch (err) {
 ...
 * Examples: 36242661232 → "36.24B", 325000000 → "325M"
 */

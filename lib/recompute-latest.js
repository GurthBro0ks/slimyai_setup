...
async function recomputeLatest(opts) {
  ...
  // Aggregate metrics from this snapshot
  ...
  if (capHint !== null && distinctMembers > capHint + 1 && !force) {
    const memberList = currentMetrics
      .map((m) => m.member_id || m.name_display)
      .sort()
      .slice(0, 50)
      .join(", ");
  ...
  for (const row of currentMetrics) {
    ...
    const corrections = correctionsMap.get(row.member_id);
    if (corrections) {
      if (corrections.sim) {
    ...
    if (payload.length) {
      ...
      await connection.query(
        `INSERT INTO club_latest
           (guild_id, member_id, name_display, sim_power, total_power, sim_prev, total_prev, sim_pct_change, total_pct_change, sim_corrected, sim_correction_reason, total_corrected, total_correction_reason, latest_at)
         VALUES ${placeholders}`,
        params
      );
    }

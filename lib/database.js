...
class Database {
  ...
  async initialize() {
    if (!this.isConfigured()) {
      console.warn(
        "[database] Skipping initialization: missing DB configuration.",
      );
      return false;
    }
  ...
  async getUserConsent(userId, _guildId = null) {
    // _guildId kept for API parity
  ...
  async setUserConsent(userId, guildIdOrConsent, maybeConsent) {
    const consent =
      typeof maybeConsent === "undefined" ? guildIdOrConsent : maybeConsent;
  ...
  async saveSnailStat(entry) {
    ...
    if (entry.guildId)
      await this.ensureGuildRecord(entry.guildId, entry.guildName);
  ...
  async saveSnailRecommendation(statId, userId, enrichment) {
    const payload = {
  ...
  async getRecentSnailStats(userId, guildId, limit = 5) {
    ...
    const rows = await this.query(
      `SELECT * FROM snail_stats
       ...
       ORDER BY created_at DESC
       LIMIT ${safeLimit}`,
      [userId, guildId || null, guildId || null],
    );
    ...
    return rows.map((row) => ({
      ...
      confidence: this.parseJSON(row.confidence, {}),
      wikiEnrichment: this.parseJSON(row.wiki_enrichment, {}),
  ...
  async markSnailStatSaved(statId) {
    await this.query(`UPDATE snail_stats SET saved_to_sheet = 1 WHERE id = ?`, [
      statId,
    ]);
  }

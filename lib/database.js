...
class Database {
  ...
  async getMemories(userId, guildId, limit = 25) {
    ...
    return rows.map(row => ({
      ...
      context: typeof row.context === 'object' && row.context !== null ? row.context : (row.context ? JSON.parse(row.context) : {}),
  ...
  async saveSnailRecommendation(statId, userId, enrichment) {
    const payload = {
      ...
      next_steps: Array.isArray(enrichment?.nextSteps) ? enrichment.nextSteps : []
    };

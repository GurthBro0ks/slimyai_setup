...
async function insertMetrics(snapshotId, entries) {
  ...
  for (const chunk of chunks) {
    ...
    for (const entry of chunk) {
      if (!entry?.memberId || !entry?.metric || !entry?.memberKey) continue;
      params.push(
        snapshotId,
        entry.memberId,
        entry.memberKey,
    ...
    // UPSERT: Use INSERT ... ON DUPLICATE KEY UPDATE to take the GREATEST value
    await database.query(
      `INSERT INTO club_metrics (snapshot_id, member_id, member_key, metric, value)
       VALUES ${placeholders}
       ON DUPLICATE KEY UPDATE
...
async function recomputeLatestForGuild(guildId, snapshotAt) {
  ...
  // Query using member_key to get union of all members (sim OR total)
  ...
  // Get previous week's metrics using member_key for proper join
    ...
    if (payload.length) {
      ...
      await connection.query(
        `INSERT INTO club_latest
           (guild_id, member_key, member_id, name_display, sim_power, total_power, sim_prev, total_prev, sim_pct_change, total_pct_change, latest_at)
         VALUES ${placeholders}`,
        params,
      );
    }

  ...
  try {
    ...
    if (err.code === "ENOENT") {
      ...
      try {
        ...
        console.error(
          "[memory] CRITICAL: Cannot create database file:",
          writeErr,
        );
        throw writeErr;
      }
    ...
    if (err instanceof SyntaxError) {
      console.error(
        "[memory] CRITICAL: Database file is corrupted (invalid JSON)",
      );
      ...
      try {
        ...
        console.warn(
          `[memory] Corrupted file backed up to: ${corruptedBackup}`,
        );
      } catch (backupErr) {
      ...
      console.warn(
        "[memory] Returning empty database - manual recovery may be needed",
      );
...
async function setChannelMode({
  ...
  console.warn(
    "[memory] setChannelMode is deprecated; use patchChannelModes instead.",
  );
...
async function listMemos({ userId, guildId, limit = 25 }) {
  ...
  return rows.slice(0, limit).map((memo) => ({
    ...
    context:
      typeof memo.context === "object" && memo.context !== null
        ? memo.context
        : {},
  }));
}

...
module.exports = {
  async criticalError(message, error, context = {}) {
    ...
    if (process.env.ERROR_WEBHOOK_URL) {
      try {
        ...
        logger.error('Failed to send error webhook', { webhookErr: webhookErr.message });
      }
    }
  },
  ...
  async warning(message, context = {}) {
    ...
    if (process.env.ERROR_WEBHOOK_URL && process.env.WEBHOOK_INCLUDE_WARNINGS === 'true') {
      try {
        const fetch = (await import('node-fetch')).default;
        await fetch(process.env.ERROR_WEBHOOK_URL, {
          method: 'POST',
        ...
        logger.error('Failed to send warning webhook', { webhookErr: webhookErr.message });
      }
    }
  },
